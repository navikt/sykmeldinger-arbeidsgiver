...
class SykmeldingAivenServiceTest :
    FunSpec({
        ...
        beforeTest {
            ...
            coEvery { pdlService.getPerson(any(), any()) } returns
                PdlPerson(Navn("fornavn", null, "etternavn"), null)
            every { database.insertOrUpdateSykmeldingArbeidsgiver(any(), any(), any()) } returns
                Unit
        }
        context("Test handling sykmelding") {
            test("Test lagring av sykmelding") {
                val consumerRecords =
                    ConsumerRecords<String, SykmeldingArbeidsgiverKafkaMessage?>(
                        mutableMapOf<
                            TopicPartition,
                            List<ConsumerRecord<String, SykmeldingArbeidsgiverKafkaMessage?>>
                        >(
                            TopicPartition("1", 1) to
                                listOf(
                                    ConsumerRecord(
                                        "topic",
                                        1,
                ...
                verify(exactly = 1) {
                    database.insertOrUpdateSykmeldingArbeidsgiver(any(), any(), any())
                }
            }
